...
export default function About() {
  ...
  return (
    <section id="about" ref={ref} className="bg-(--color-bg-cream) relative overflow-hidden" aria-label="About Venant Cutlery">
      ...
      <div className="absolute top-15 left-10 w-1 h-15 bg-(--color-accent-pink) rounded-xs -rotate-20 opacity-70" />
      <div className="absolute top-20 left-15 w-1 h-10 bg-(--color-accent-pink) rounded-xs -rotate-20 opacity-50" />
      <div className="absolute top-25 right-15 w-1 h-15 bg-(--color-accent-pink) rounded-xs rotate-20 opacity-70" />
      <div className="absolute top-15 right-10 w-1 h-10 bg-(--color-accent-pink) rounded-xs rotate-20 opacity-50" />
      <div className="absolute top-[40%] left-7.5 w-4 h-4 rounded-full bg-(--color-accent-yellow) opacity-80" />
      <div className="absolute top-[60%] right-7.5 w-5 h-5 rounded-full bg-(--color-accent-yellow) opacity-60" />

      <div className="container section relative z-1">

        {/* Header */}
        <div className="reveal text-center mb-18">
          <p className="font-body text-sm font-bold tracking-[0.15em] uppercase text-(--color-primary) mb-4 opacity-60">
            Our Story
          </p>
        ...
        {/* Two column layout */}
        <div className="about-grid grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-14 items-center">
          {/* Image */}
          <div className="reveal-left relative">
            <Image
              ...
              className="w-full h-auto object-cover rounded-lg block transition-transform duration-400 hover:scale-[1.03]"
            />
            {/* Overlaid quote */}
            <div className="absolute bottom-5 left-5 right-5 bg-[rgba(0,36,68,0.95)] backdrop-blur border-l-4 border-(--color-brand-gold) rounded-md p-[18px_22px]">
              <div className="text-(--color-brand-gold) mb-1">
                <ChatTeardropText size={24} weight="fill" />
              </div>
              <p className="font-display italic text-lg text-(--color-brand-gold) leading-[1.4]">
                A rich, spicy, and unforgettable experience made just for you.
              </p>
              <p className="font-body text-xs text-white/50 mt-2">— Venant Cutlery</p>
            </div>
          </div>
          ...
          <div className="reveal-right">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {features.map((f, i) => {
                ...
                return (
                  <div key={f.title}
                    className={`reveal delay-${(i+1)*100} rounded-md py-5.5 px-4.5 transition-transform duration-250 cursor-default hover:-translate-y-1.5! ${even ? "bg-(--color-primary)" : "bg-(--color-bg-lime)"}`}
                  >
                    <div className={`mb-2.5 ${even ? "text-(--color-brand-gold)" : "text-(--color-primary)"}`}>
                      <Icon size={32} weight="fill" />
                    </div>
                    <h3 className={`font-display font-bold text-[17px] mb-1.5 ${even ? "text-white" : "text-(--color-primary)"}`}>{f.title}</h3>
